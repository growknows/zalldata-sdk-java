...
通过***IdTypeEnum***对不同的ID进行映射，需要开发者导入项目，并在上报数据的properties中放入***$distinctIdType***及其对应的值（具体使用见下文）：
...
public static void main(String[] args) {
     ...
     properties.put("$distinctIdType", IdTypeEnum.DEVICE_ID.getType());

     // 记录用户登录事件
     za.track(distinctId, true, "UserLogin", properties);
...
// ID映射关系(必要条件)
Map<String, Object> properties = new HashMap<String, Object>();
properties.put("$distinctIdType", IdTypeEnum.LOGIN_ID.getType());
properties.put("$originalIdType", IdTypeEnum.DEVICE_ID.getType());

// 用户注册/登录时，将用户登录 ID 与匿名 ID 关联
za.trackSignUp(registerId, anonymousId, properties);
```
...
- 调用***trackSignUp()***时必须传入***\$distinctIdType***和***\$originalIdType***对用户ID进行类型映射，否则会导致两个用户的出现
...
// 设置用户渠道属性（AdSource）为 "App Store"
za.profileSetOnce(distinctId, true, properties);

// 再次设置用户渠道属性（AdSource），设定无效，属性 "AdSource" 的值仍为 "App Store"
properties.put("AdSource", "Search Engine");
za.profileSetOnce(distinctId, true, properties);
```
...
properties.put("GamePlayed", 1);
// ID映射关系(必要条件)
properties.put("$distinctIdType", IdTypeEnum.DEVICE_ID.getType());

// 设置用户游戏次数属性（GamePlayed），将次数累加1次
za.profileIncrement(distinctId, true, properties);
```
...
批量发送数据的 **Consumer**，当数据达到指定的量时（默认50条，最多可指定50条），才将数据进行发送。也可以调用 **flush()** 方法去强制发送。

线上环境一般使用该实例，若业务数据对于时效性要求较高，可将指定数量设置为1。
...
##### 7.1.3. ConsoleConsumer

用于将数据输出到特定 Writer，一般用于在生产环境的 Java 程序中处理历史数据。
...
**7.1.4.LoggerConsumer**

批量实时写本地文件，文件默认以天为分隔，**logDirectory**为写入本地的文件夹地址。 
